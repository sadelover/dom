...
import s from './ReportHistoryView.css'
...
import { DatePicker,Row,Col, Form ,Button ,Modal ,Select ,message,Spin,Input,Layout,Table,Pagination} from 'antd';
...
class FormWrap extends React.Component {
    ...
    deleteReport(Id){
        ...
        Modal.confirm({
            title : '是否确定删除该报表？',
            content : '点击"确定"按钮删除该报表。',
            onOk(){
                http.post('/report/delete',{
                    id:Id
                }).then(
                    data=>{
                        if(data.err==0){
    ...
    render() {
        ...
        return (
                <div  ref={this.saveContainerRef} style={{overflow:"hidden"}}>
                                ...
                                <Table
                                    columns={[{
                                        ...
                                        width: '6%',
                                            ...
                                            width:'6%',
                                            render:(text,record)=>{
                                                return(<div><Button onClick={()=>this.download(record.url)}>下载</Button><Button style={{marginLeft:20}} onClick={()=>this.deleteReport(record.id)}>删除</Button></div>)
                                            }
                                        }]}
 ...
 * @extends {Widget}
 */
class ReportHistoryComponent extends Widget {
