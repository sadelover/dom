...
import { Table, Select, DatePicker, Button, Form, Input, message, Spin } from 'antd'
import s from './EnergyView.css'
import cx from 'classnames'
import ChartView from './ChartView'
import PieChart from './PieChatView'
...
const registerInformation = {
    type: 'energy',
    name: '能源组件',
    description: "生成能源组件，覆盖canvas对应区域",
}
...
class FormWrap extends React.Component {
    ...
    getEquipmentOptGroup = () => {
        ...
        if (this.props.config.customGroups && this.props.config.customGroups.length != 0) {
            this.props.config.customGroups.map((item, index) => {
    ...
    getEquipmentList = (classify, equipmentTag) => {
        ...
        // 取出全局配置中与分组相关的设备GroupList(如：ChGroupList:1,2,3 or 1|2|3,4)
        let equipmentGroupList = null
        let equipmentGroupListArr = []
        let equipmentGroupListStr = null
        let equipmentList = [];

        equipmentList.push({ point: classify + "GroupPowerTotal", cnPoint: equipmentTag + '总功耗', classify })
        for (var key in currentConfig) {
            if (currentConfig.hasOwnProperty(key) && (classify + "GroupList") == key) {
                // key值需要匹配当前的项
                equipmentGroupList = currentConfig[key]
            }
        }

        //将GroupList字段，剔除逗号和竖线，数组去重后，即为设备编号清单
        let reg = /|/;

        if (equipmentGroupList !== null) {
            //有竖线，先把每个竖线改为逗号，然后再转为数组
            if (reg.test(equipmentGroupList)) {
                equipmentGroupListStr = equipmentGroupList.replace(/\|/g, ",")
                console.log(equipmentGroupListStr);
            }
            let arr = []
            arr = equipmentGroupListStr.split(',')
            console.log(typeof (arr))
            //去重
            equipmentGroupListArr = Array.from(new Set(arr));

            if (equipmentGroupListArr.length != 0) {
                let equipment = null, cnPoint = '';
                equipmentGroupListArr.forEach(item => {
                    if (Number(item) < 10) {
                        equipment = classify + "PowerTotal" + "0" + item
                    } else {
                        equipment = classify + "PowerTotal" + item
                    }

                    cnPoint = equipmentTag + item + '号功耗'
 ...
 * energy主页面
 */
class EnergyView extends Widget {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            color: [],
            pieData: [],
            roomName: this.props.config.RoomName ? this.props.config.RoomName : '',
            price: this.props.config.price ? this.props.config.price : "1",
            extraEquipment: [],
            powerTotalPiont: '',
            initName: '冷机'
        }
        ...
        http.post('/project/getConfig')
            .then(
                data => {
                    if (data.status) {
                        _this.setState({
                            //globalConfig : JSON.stringify(data.data)
    ...
    getExtraEquipment = () => {
        ...
        let timeStart = moment(energyDateStr).startOf('day').format(TIME_FORMAT),
            ...
            _this = this;
        // 额外的设备今日总用电量
        this.props.config.customGroups.map((row, index) => {
            let pointList = []
            ...
            http.post('/get_history_data_padded', {
                ...
                data => {
                    ...
                    if (this.props.config.customGroups.length == (index + 1)) {
                        console.log("leo" + str)
                        _this.setState({
                            extraEquipment: str,
                            loading: false
    ...
    getComponent = () => {
        const { color, pieData } = this.state
        return pieData.map((item, index) => {
            return (
                <div key={item.name} style={{ backgroundColor: color[index] }} >
                    {item.name}
                    {item.value}
                </div>
            )
    ...
    getContent() {
        ...
        return (
            <div>
                ...
                {
                    localStorage.getItem('serverOmd') == "best" || localStorage.getItem('serverOmd') == "persagy" ?
                        <div className={s['container']} style={{ width: width, height: height, left: left, top: top }} >
                            <div className={s['header']} style={headerBgStyle}>
                                <DatePicker
                                    onChange={this.handleChangeDate}
                                    allowClear={false}
                                    value={moment(energyDateStr)}
                                    className="EnergyViewSelectStyle"
                                    style={{ width: "115px" }}
                                />
                            </div>
                            <div className={s['top2']}>
                                <div className={s['top-left2']} >
                                    {
                                        this.state.loading ?
                                            ...
                                            :
                                            <div>
                                                <h2 style={{ color: "rgba(51,57,64,1)", marginLeft: "20px" }}>能源参数</h2>
                                                <div className={s['top-left-left']} >
                                                    <div className={s['top-left-item2']} >
                                ...
                                <div className={s['top-right2']} >
                                    <h2 style={{ color: "rgba(51,57,64,1)", marginLeft: "20px" }}>逐时能耗</h2>
                                    <ChartView
                            ...
                            <div className={s['bottom2']}>
                                <div className={s['energy2']} >
                                    <div className={s['energy-title2']}>
                                        <h2 className={s['energy2-title2-h2']}>各时段能耗统计</h2>
                                        <Button onClick={this.selectDay} type={this.state.selectTimes === 'day' ? "primary" : 'default'} className={s['energy2-title2-btn']}>日</Button>
                                        {/* <Button onClick={this.selectWeek} type={this.state.selectTimes === 'week' ? "primary" : 'default'} >周</Button> */}
                                        {/* <Button onClick={this.selectMonth} type={this.state.selectTimes === 'month' ? "primary" : 'default'} >月</Button> */}
                                    </div>
                                    <div className={s['energy-details']} >
                                        ...
                                        {/* <div className={s['energy-data']} >
                                {this.getComponent()}
                            </div>  */}
                                    </div>
                                </div>
                                <div className={s['equipment2']}>
                                    <TableView
                        ...
                        <div className={s['container']} style={{ width: width, height: height, left: left, top: top, color: "#CADCE7" }} >
                            ...
                            <div className={s['top']}>
                                ...
                                <div className={s['top-right']} style={{ overflow: 'hidden' }}>
                                    ...
                                    <div style={{ display: 'inline-block', marginLeft: 20 }}>
                                        ...
                                        {
                                            this.props.config.customGroups && this.props.config.customGroups.length != 0 ?
                                                this.props.config.customGroups.map((item, index) => {
