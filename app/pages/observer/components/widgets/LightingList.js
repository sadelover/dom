...
const formItemLayout = {
  ...
  class CommandWindow extends React.Component{
    constructor(props) {
     ...
     onCancel(){
        this.props.LightSwitch(false)
     }
     handleSubmit(){
        const {text,record,value,index,BackValue,BackPoint,type} = this.props
        this.props.LightSwitch(false)
        ...
        http.post('/pointData/setValue',{
            ...
            data=>{
                if(data.err===0) {
                  this.checkPointRefresh(StartTime,text,value,index,1,BackValue,type)
                }
            }
        )
     }
    ...
    render(){
        const {value,text,index,record,BackValue,BackPoint,LightingSwitchVisiable,type,
            LightSwitch
        } = this.props
        return(
            <Modal
                title='确认指令'
                width={500}
                visible={LightingSwitchVisiable}
...
class LightingControlList extends React.Component {
            ...
            columns:[
                ...
                },
                {
                    ...
                    render:(text,record)=>{
                        ...
                        }else if(record['OnOffSetting']!=record['OnOff']){
                            return (    
                                <div className={L['Center']}  style={{fontSize:"15px"}}>
    ...
    initTitle(colName){
        return <span style={{display:'table',textAlign:'center'}}>{colName}</span>
    }
    componentDidMount(){
        ...
        if( this.props.config.groupPaneEnable!=undefined&&this.props.config.groupPaneEnable==0){
            ...
            document.getElementById('Light_Select').childNodes[indexId].style.backgroundColor='#2ea2f8' 
        }
    }
    ...
    Onchange(value){
        let Arr = this.props.LightingControlDataSource.filter((row,index)=>{
    ...
    selectList(name,list,index,e){
        ...
        let arr = document.getElementById('Light_Select').childNodes
    ...
    render() {
            const {
                LightData,LightingControlDataSource,Lightloading,LightingloadingVisiable,
                LightingSwitchVisiable,LightSwitch,LightSetting,LightingSettingVisiable
            } = this.props
                   ...
                   <Layout>
                            ...
                            <Layout>
                                ...
                                <Content>
                                    ...
                                    rowKey="id"
                                    align='center'
                                    dataSource={this.state.localDateSource.length>0?this.state.localDateSource:this.props.LightingControlDataSource}
                                    loading={this.props.LightingloadingVisiable}
                                    scroll={{
 ...
 * @extends {Widget}
 */
class LightControlListComponent extends Widget {
    constructor(props){
        super(props)
        this.state = {
            style : {},
...
export default  LightControlListComponent