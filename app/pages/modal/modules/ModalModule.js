...
export const SHOW_POINT_MODAL = 'modal.SHOW_POINT_MODAL';
export const SHOW_OBSERVER_MODAL = 'modal.SHOW_OBSERVER_MODAL';
export const SHOW_OPTIMIZE_VALUE_MODAL = 'modal.SHOW_OPTIMIZE_VALUE_MODAL';
export const TIME_PICKER_MODAL = 'modal.TIME_PICKER_MODAL';
export const VALUE_LOADING = 'modal.VALUE_LOADING';
export const GET_POINT_INFO = 'modal.GET_POINT_INFO';
export const GET_DICTBINDSTRING = 'modal.GET_DICTBINDSTRING';
export const OPERATE_SWITCH_MODAL = 'modal.OPERATE_SWITCH_MODAL';
export const SHOW_SWITCH = 'modal.SHOW_SWITCH';
export const HIDE_SWITCH = 'modal.HIDE_SWITCH';
export const SHOW_CHECKBOX = 'modal.SHOW_CHECKBOX';
export const HIDE_CHECKBOX = 'modal.HIDE_CHECKBOX';
export const SHOW_TEXT = 'modal.SHOW_TEXT';
export const HIDE_TEXT = 'modal.HIDE_TEXT';
export const SHOW_RADIO = 'modal.SHOW_RADIO';
export const HIDE_RADIO = 'modal.HIDE_RADIO';
export const SHOW_SELECT = 'modal.SHOW_SELECT';
export const HIDE_SELECT = 'modal.HIDE_SELECT';
export const SHOW_ALARM = 'modal.SHOW_ALARM';
export const HIDE_ALARM = 'modal.HIDE_ALARM';
export const OBSERVER_MODAL_DICT = 'modal.OBSERVER_MODAL_DICT';
export const UPDATE_OPERATE_DATA = 'modal.UPDATE_OPERATE_DATA';
export const TABLE_LOADING = 'modal.TABLE_LOADING';
...
export function showObserverModal(modalProps) {
	return showModal(modalTypes.OBSERVER_MODAL, modalProps);
}
			...
			http.post('/get_realtimedata', {
				...
				data => {
					if (data.length != 0) {
						...
						if (pointInfo.param10 != undefined && pointInfo.param10 != "" && pointInfo.param10.indexOf('|') != -1) {
							if (pointInfo.param10.slice(pointInfo.param10.indexOf('|') + 1) > JSON.parse(window.localStorage.getItem('userData')).role) {
								Modal.info({
									title: '提示',
									content: '用户权限不足'
								})
...
export function operateSwitch(idCom, setValue, description) {
	let setValueString = setValue.toString()
	//往后台传的都是数组格式
	let valueList = []

	if (setValueString != undefined && setValueString.indexOf(',') != -1) {
		//多个值（一个按钮要给多个点位赋值情况）
		valueList = setValueString.split(",")
	} else {
		if (setValueString != undefined) {
			//单个值（常见一个按钮给一个点赋值情况）
	...
	let pointName = []
	if (idCom != undefined && idCom.toString().indexOf(',') != -1) {
		pointName = idCom.toString().split(",")
	} else {
		if (idCom != undefined) {
	...
	if (valueList.length < pointName.length) {
		for (let i=valueList.length; i<pointName.length;i++) {
			valueList.push(Number(valueList[length-1]))
		}
	}

	return function (dispatch, getState) {
		let modal = getState().modal
							...
							pointData => {
								if (pointData[0]) {
									...
									http.post('/operationRecord/add', {
										...
										"content": description? description :`将${idCom}的点值修改为${setValue}`,
...
//优化选项设置
export function checkboxSetting(idCom, setValue, text, unsetValue, currentValue,desc) {
	let pointName = [idCom]
...
export function optimizeSetting(data, idCom) {
	...
	return function (dispatch, getState) {
		...
		return http.post('/pointData/setValue', {
			...
			valueList: setValue,
			source: localStorage.getItem('userInfo') && JSON.parse(localStorage.getItem('userInfo')).name ? JSON.parse(localStorage.getItem('userInfo')).name : '',
			waitSuccess: localStorage.getItem('hideLoading') == 1?0:1,
			addOperationLog: 1,
			userName: localStorage.getItem('userInfo') && JSON.parse(localStorage.getItem('userInfo')).name ? JSON.parse(localStorage.getItem('userInfo')).name : '',
		}).then(
			serverData => {
				if (serverData.err === 0) {
					...
					ObserverScreen.prototype.refreshData({data:[{name:pointName[0],value:setValue[0]}]})
					//获取点名的对应信息
					// http.post('/analysis/get_point_info_from_s3db', {
					// 	pointList: [idCom]
					// }).then(
...
export function tableCellSetting(data, idCom, defaultValue) {
	return function (dispatch, getState) {
		const { firstKey, secondKey, pointvalue } = getState().modal.props
		const { customList } = Object.assign({}, { customList: getState().observer.customList })
		...
		return http.post('/pointData/setValue', {
			...
			res => {
				if(res.err == 0){
					...
					ObserverScreen.prototype.refreshDataFloat({data:[{name:pointName[0],value:setValue[0]}]})
				}else{
					if (localStorage.getItem('hideLoading') != 1) {
						dispatch(valueLoading(false))
					}
		...
		return function (dispatch, getState) {
			...
			return http.post('/pointData/setValue', {
				...
				serverData => {
					...
					if (serverData.err === 0) {
						ObserverScreen.prototype.refreshData({data:[{name:idCom,value:setValue.toString()}]})
					} else if (serverData.err > 0) {
						message.error(serverData.msg)
					}
				}
					...
					if (diffData != value) {
						...
						if (localStorage.getItem('hideLoading') == 1) {
							http.post('/operationRecord/add', {
								"userName": localStorage.getItem('userInfo') && JSON.parse(localStorage.getItem('userInfo')).name ?
									JSON.parse(localStorage.getItem('userInfo')).name : '',
								"content": `将${pointName[0]}点值修改为${value}`,
								"address": localStorage.getItem('serverUrl')
							}).then(
							data => {
							...
							if (getState().modal.textVisible) {
								...
								if (getState().modal.radioVisible) {
									addOperation('/operationRecord/addChangeValue', {
										"userName": localStorage.getItem('userInfo') && JSON.parse(localStorage.getItem('userInfo')).name ?
											JSON.parse(localStorage.getItem('userInfo')).name : '',
										"pointName": pointName[0],
									...
									if (getState().modal.selectVisible) {
...
const ACTION_HANDLERS = {
	...
	[HIDE_TEXT]: (state) => {
		return { ...state, textVisible: false, modalConditionDict: { status: false, description: '' } }
	},
