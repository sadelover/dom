...
const ModifyModalView = Form.create()(
    class extends React.PureComponent{
        ...
        onOk(e){
            ...
            //   if (!err&&values.upload!=undefined) {
                _this.props.modifyList(values)
        ...
        render(){      
            ...
            let system_name,system_desc = ''
            if(data.length!=0){
                system_name = data[0].system_name
                system_desc = data[0].system_desc
            }
            //是否显示新增页面的弹窗
            let visible = modalTypes.EQUIPSYSTEM_MODIFY_MODAL=== modal.type ? true : false
            return (
                <Modal 
                    title='新建'
                    visible={visible}
